...
VALUES (
  (SELECT credencial_FIA FROM Pessoa 
   WHERE nome = 'Juan Manuel Correa' AND data_nascimento = DATE '1990-05-12'),
  'LS-JMC001',
  'Race Steward'
);
...
VALUES (
  (SELECT credencial_FIA FROM Pessoa 
   WHERE nome = 'Fabiana Flosi' AND data_nascimento = DATE '1988-03-25'),
  'LS-FF002',
  'Race Control Officer'
);

INSERT INTO Funcionario_FIA (credencial_FIA_pessoa, licenca_staff, cargo_exercido)
VALUES (
  (SELECT credencial_FIA FROM Pessoa 
   WHERE nome = 'Niels Wittich' AND data_nascimento = DATE '1968-07-10'),
  'LS-NW003',
  'Race Director'
);
